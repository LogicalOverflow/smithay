...
impl<A: GraphicsApi> GpuManager<A> {
    ...
    pub fn new(api: A) -> Result<GpuManager<A>, Error<A, A>> {
        ...
        Ok(GpuManager {
            ...
            span,
            id: RendererId(next_renderer_id()),
        })
    }
        ...
        <<A::Device as ApiDevice>::Renderer as ExportMem>::TextureMapping: 'static,
    {
        match buffer_type(buffer) {
            Some(BufferType::Dma) => {
                ...
                let src_node = import_on_src_node(
                    &dmabuf,
                    Some(damage),
                    self.id.0,
                    &mut texture,
                ...
                if src_node != target_node {
                    ...
                    let api_textures = texture_internal
                        .textures
                        .get_mut(&(self.id.0, TypeId::of::<A>()))
                        .unwrap();
...
struct MultiTextureInternal {
    textures: HashMap<(usize, TypeId), HashMap<DrmNode, GpuSingleTexture>>,
    size: Size<i32, BufferCoords>,
...
impl MultiTexture {
        ...
        <<A::Device as ApiDevice>::Renderer as Renderer>::TextureId: 'static,
    {
        ...
        let textures = tex.textures.entry((id, TypeId::of::<A>())).or_default();
        ...
        <<T::Device as ApiDevice>::Renderer as ExportMem>::TextureMapping: 'static,
    {
        ...
        let textures = tex.textures.entry((id, TypeId::of::<R>())).or_default();
    ...
    <<T::Device as ApiDevice>::Renderer as Renderer>::Error: 'static,
{
    ...
    fn render_texture_from_to(
        ...
        let sync = texture.needs_synchronization::<R>(self.id, &self.node);
        if let Some(texture) = texture.get::<R>(self.id, &self.node) {
            self.damage.extend(damage.iter().copied().map(|mut rect| {
                rect.loc += dst.loc;
                rect
            }));
    ...
    <<T::Device as ApiDevice>::Renderer as Renderer>::Error: 'static,
{
    ...
    fn import_shm_buffer(
        ...
        texture.insert_texture::<R>(self.id, *self.render.node(), shm_texture);
        Ok(texture)
    }
    ...
    <<T::Device as ApiDevice>::Renderer as Renderer>::Error: 'static,
{
    ...
    fn import_memory(
        ...
        texture.insert_texture::<R>(self.id, *self.render.node(), mem_texture);
        Ok(texture)
    }
    ...
    fn update_memory(
        ...
        let mem_texture = texture
            .get::<R>(self.id, self.render.node())
        ...
        Some(node) => {
            if node == *render.node() {
                let imported = render
                    ...
                    .map_err(Error::Render)?;
                texture.insert_texture::<R>(id, node, imported);
                ...
                let imported = target
                    ...
                    .import_dmabuf(dmabuf, damage)
                    .map_err(Error::Target)?;
                texture.insert_texture::<T>(id, node, imported);
        ...
        if src_node == *self.render.node() {
                ...
                .is_some_and(|target| src_node == *target.device.node())
            {
                ...
                let (mut src_api_textures, other_api_textures) = texture_internal
                    .textures
                    .iter_mut()
                    .partition::<Vec<_>, _>(|((id, type_), _v)| {
                        *id == self.id && *type_ == TypeId::of::<T>()
                    });
                let mut target_api_textures = other_api_textures
                    .into_iter()
                    .find(|((id, type_), _)| *id == self.id && *type_ == TypeId::of::<R>());
